...
void SFML_BulletSpriteObject::setMoveDirection(sf::Vector2f directionVector, bool rotate)
{
	...
	if (rotate)
	{
		const float PI = static_cast<float>(3.1415);
...
void SFML_BulletSpriteObject::applyTexture(sf::Texture* texture)
{
	...
	m_spriteOrigin = sf::Vector2f(static_cast<float>(m_spriteTexture->getSize().x / 2), static_cast<float>(m_spriteTexture->getSize().y / 2));
